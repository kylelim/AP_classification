...
class Eval:
    ...
    def eval_Testscore(self) -> float: #This function is the same as the eval_score function, only the dataset is changed to testset
        """
        Calculate the score on each row and return the mean score.
        :return: The mean score
        """
        # filter out the discarded samples
        self.testset = self.testset[(self.testset['prediction'] != 'Discarded') &
                                    (self.testset['annotation'] != 'Discarded')]
        self.testset = self.score_func(self.testset)
        self.mean_score = self.testset['score'].mean()
        return self.mean_score
    ...
    def add_history(self, prompt: str, task_description: str): #When eval method is accuracy, the confusion matrix is a good indicator of the performance of the model
        """
